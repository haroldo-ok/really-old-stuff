...
unsigned char map[] = {,
...
char collide_actor_map(unsigned char *map, actor *act) {
	return collide_rect_map(map, act->x, act->y, 15, 15);
}
...
char shakeoff_actor_horiz(actor *act) {
	...
	while (spd_x_cnt > 0) {
		...
		if (!collide_rect_map(map, x, y, 15, 15)) {
			act->x = x;
			act->spd_x.w = 0;
			return 1;
		}
...
void draw_half_tile(int x, int y, int tile) {
	...
	*(buffer+1) = tle+1;
	...
	set_bkg_map(buffer, x &0x1F, y & 0x1E, 1, 2);
}

void draw_map_coord(int x, int y) {
	unsigned int tile = get_map_tile(map, x >> 1, y >> 1);

	tile <<= 2;
	draw_half_tile(x, y, tile);
}
...
void draw_bkg_scroll(scroll_data *scroll) {
	int delta_x = asr(scroll->x - scroll->prev_x, 3);
	int delta_y = asr(scroll->y - scroll->prev_y, 3);
	...
	if (delta_x != 0) {
		if (delta_x < 0) {
			for (i = delta_x, tx2 = tx; i; i++, tx2++) {
				for (j = 0, ty2 = ty; j < VISIBLE_BKG_HEIGHT; j += 2, ty2 += 2) {
					draw_map_coord(tx2, ty2);
				}
			}
		} else {
			for (i = delta_x, tx2 = tx + VISIBLE_BKG_WIDTH - 1; i; i--, tx2--) {
				for (j = 0, ty2 = ty; j < VISIBLE_BKG_HEIGHT; j += 2, ty2 += 2) {
					draw_map_coord(tx2, ty2);
				}
			}
	...
	if (delta_y != 0) {
		if (delta_y < 0) {
			for (i = VISIBLE_BKG_WIDTH, tx2 = tx; i; i--, tx2++) {
				for (j = delta_y, ty2 = ty; j < 0; j += 2, ty2 += 2) {
					draw_map_coord(tx2, ty2);
				}
			}
		} else {
			for (i = VISIBLE_BKG_WIDTH, tx2 = tx; i; i--, tx2++) {
				for (j = delta_y, ty2 = ty + VISIBLE_BKG_HEIGHT; j > 0; j -= 2, ty2 -= 2) {
					draw_map_coord(tx2, ty2);
				}
			}
		}
